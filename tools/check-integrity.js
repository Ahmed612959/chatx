...
const [htmlPath = path.join(root, 'index.html'),
       cssPath = path.join(root, 'styles.css'),
       jsPath = path.join(root, 'script.js')] = process.argv.slice(2);
...
for (const u of refs) {
    ...
    const candidates = u.startsWith('/')
        ? [path.join(root, u), path.join(root, 'public', u)]
...
if (js) {
    ...
    for (const m of js.matchAll(/classList\.(?:add|toggle|replace)\(\s*([^)]*)\)/g)) {
        for (const c of m[1].matchAll(/['"]([A-Za-z_][\w-]*)['"]/g)) {
            const cls = c[1];
            if (!cssClasses.has(cls) && !seen.has(cls)) { seen.add(cls); warnings.push(`script.js بيضيف الكلاس "${cls}" ومفيش له قاعدة في styles.css`); }
        }
    }
}
