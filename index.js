...
// ====================== نموذج فيديوهات شرح المنصة (Video Lessons) ======================
// فيديوهات شرح بيرفعها الأدمن بس (شرح استخدام Chat X)، وبتتعرض لأي زائر
// من غير ما يحتاج يسجل دخول — عكس المكتبة المشتركة اللي محتاجة تسجيل دخول.
const videoLessonSchema = new mongoose.Schema({
    ...
    publicId: { type: String, required: true },
    size: { type: Number },
    duration: { type: Number }, // بالثواني — اختياري، بيتحسب في الفرونت إند وقت الرفع
    views: { type: Number, default: 0 },
    uploadedBy: { type: String },
    createdAt: { type: Date, default: Date.now }
});
const VideoLesson = mongoose.models.VideoLesson || mongoose.model('VideoLesson', videoLessonSchema);
...
// ====================== فيديوهات شرح المنصة (Video Lessons) ======================
// رابط رفع فيديو موقّع مباشرة على R2 (الأدمن فقط) — نفس فكرة /api/files/upload-url
// بالظبط، بس في فولدر video-lessons ومحدود على صيغ الفيديو المعروفة.
app.post('/api/video-lessons/upload-url', verifyToken, isAdmin, async (req, res) => {
    try {
        const { fileName } = req.body;
        if (!fileName) return res.status(400).json({ error: 'اسم ملف الفيديو مطلوب' });

        const ext = String(fileName).split('.').pop().toLowerCase();
        const allowedExts = ['mp4', 'webm', 'mov', 'mkv', 'm4v'];
        if (!allowedExts.includes(ext)) {
            return res.status(400).json({ error: 'صيغة الفيديو غير مدعومة (المسموح: mp4, webm, mov, mkv, m4v)' });
        }

        const safeName = `${Date.now()}-${sanitizeFileName(fileName)}`;
        const path = `video-lessons/${safeName}`;

        // من غير ContentType هنا عمدًا (زي /api/files/upload-url بالظبط) —
        // المتصفح بيبعت Content-Type الفيديو تلقائي مع الـ PUT من غير ما نوقّعه،
        // فبنتفادى مشكلة SignatureDoesNotMatch.
        ...
        res.status(500).json({ error: 'خطأ في إنشاء رابط رفع الفيديو' });
    }
});

// حفظ بيانات فيديو الشرح بعد رفعه مباشرة على R2 (الأدمن فقط)
app.post('/api/video-lessons', verifyToken, isAdmin, async (req, res) => {
    try {
        await connectToDatabase();
        const { title, description, creatorName, url, publicId, size, duration } = req.body;

        if (!title || !String(title).trim()) return res.status(400).json({ error: 'عنوان الفيديو مطلوب' });
        if (!creatorName || !String(creatorName).trim()) return res.status(400).json({ error: 'اسم صاحب الفيديو مطلوب' });
        if (!url || !publicId) return res.status(400).json({ error: 'بيانات الفيديو المرفوع ناقصة' });
...
// عرض كل فيديوهات الشرح — متاح لأي زائر من غير تسجيل دخول، عشان أي حد يقدر يتفرج
...
// زيادة عداد المشاهدات — متاح كمان لأي زائر (مجرد إحصائية بسيطة، مش حساسة)
app.post('/api/video-lessons/:id/view', async (req, res) => {
    try {
        await connectToDatabase();
        await VideoLesson.findByIdAndUpdate(req.params.id, { $inc: { views: 1 } });
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: 'تعذر تحديث عداد المشاهدات' });
    }
});

// حذف فيديو شرح (الأدمن فقط) — من قاعدة البيانات ومن تخزين R2 مع بعض
app.delete('/api/video-lessons/:id', verifyToken, isAdmin, async (req, res) => {
    try {
        await connectToDatabase();
        const video = await VideoLesson.findById(req.params.id);
        if (!video) return res.status(404).json({ error: 'الفيديو غير موجود' });

        try { await deleteFromSupabase(video.publicId); } catch (e) {
            console.error('⚠️ تعذر حذف ملف الفيديو من R2 (هيتم حذف السجل من الداتابيز برضو):', e.message);
        }
        await VideoLesson.deleteOne({ _id: req.params.id });

        res.json({ success: true });
    } catch (error) {
        console.error('❌ خطأ في حذف فيديو الشرح:', error);
        res.status(500).json({ error: 'خطأ في حذف الفيديو' });
    }
});
