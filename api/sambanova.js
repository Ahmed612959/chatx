...
export default async function handler(request) {
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'تعذر الوصول لـ SambaNova' }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    await reportApiUsage('sambanova', body.length);
        ...
        try {
          ...
          try {
            controller.enqueue(new TextEncoder().encode(
              `data: {"error":{"message":"انقطع الاتصال بـ SambaNova أثناء الرد"}}\n\n`
            ));
          } catch (e) {}
          controller.close();
        }
