...
export default async function handler(request) {
  try {
    ...
    if (!MISTRAL_API_KEY) {
      return new Response(JSON.stringify({ error: 'MISTRAL_API_KEY غير مضبوط في Environment Variables' }), {
        status: 500,
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'تعذر الوصول لـ Mistral' }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' }
      });
    }
        ...
        try {
          ...
          try {
            controller.enqueue(new TextEncoder().encode(
              `data: {"error":{"message":"انقطع الاتصال بـ Mistral أثناء الرد"}}\n\n`
            ));
          } catch (e) {}
          controller.close();
        }
